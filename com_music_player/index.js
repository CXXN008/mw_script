...
const a_src = com.querySelector('a').href
...
let DEFAULTS = {
	borderColor: '#8ebbc4',
	playedColor: '#ffffff',
	backgroundColor: 'rgba(142,187,196,.3)',
	iconColor: '#ffffff',
	borderWidth: 2,
	size: 48,
...
class CircleAudioPlayer {
		...
		if (deltaTime >= animTime) {
			...
			for (let i = 0; i < from.length; i++) {
				...
				for (let j = 0; j < from[i].length; j++) {
					...
					current[i][j][0] =
						from[i][j][0] + (to[0] - from[i][j][0]) * perc
					current[i][j][1] =
						from[i][j][1] + (to[1] - from[i][j][1]) * perc
				}
			}
	...
	setSize(size) {
		...
		let playLeft = Math.cos((pi / 3) * 2) * (iconSize / 2) + this._halfSize
		let playRight = iconSize / 2 + this._halfSize
		let playHalf = (playRight - playLeft) / 2 + playLeft
		let top = this._halfSize - Math.sin((pi / 3) * 2) * (iconSize / 2)
		let bottom = this.size - top
		let pauseLeft = this._halfSize - iconSize / 3
...
params['audio'] = a_src
// init
let cap = new CircleAudioPlayer(params)
com.innerHTML =''
cap.appendTo(com)
